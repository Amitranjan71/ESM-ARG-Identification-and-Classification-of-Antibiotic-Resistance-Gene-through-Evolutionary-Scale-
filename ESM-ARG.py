...
def main():
    parser = argparse.ArgumentParser(prog='ESM-ARG')
    ...
    plm_arg_p.add_argument('--arg-model', default='path/models/arg_model.pkl', help='Model for ARG identification')
    plm_arg_p.add_argument('--cat-model', default='path/models/cat_model.pkl', help='Model for resistance category classification')
    plm_arg_p.add_argument('--cat-index', default='path/models/Category_Index.csv', help='File for the resistance category index')
    ...
    plm_arg_t = subparsers.add_parser("train", help="Retrain the ESM-ARG models")
    plm_arg_t.add_argument('-i', '--input-file', required=True, help='Input file (Fasta input file)')
    plm_arg_t.add_argument('--arg-model', default='path/models/arg_model.pkl', help='Model for ARG identification')
    plm_arg_t.add_argument('--cat-model', default='path/models/cat_model.pkl',
                         help='Model for resistance category classification')
    plm_arg_t.add_argument('--cat-index', default='path/models/Category_Index.csv',
                         help='File for the resistance category index')
    plm_arg_t.add_argument('--min-seq', default=10, type=float, help='Minimal sequence number for the category training [Default: 50]')
